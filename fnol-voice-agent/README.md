...
| **Carriers can only be added in the case management system, never in the app** | Letting a reviewer add a missing carrier from inside the app is an obvious feature and saves them about two minutes. Nobody asked for it and I left it out on purpose. Every other solution reads its carriers from the case management system, so a carrier that exists only in this app is invisible to all of them, and a single source of truth erodes one convenient exception at a time. Two minutes of a reviewer's time does not buy that. | A slower path when a carrier really is missing, and a sensible feature that reads as an omission until someone asks why. |
